...
export class Table extends BaseVisualization<TableState> {
  ...
  makeMeasuresRenderer(essence: Essence, formatters: Formatter[], hScales: d3.scale.Linear<number, number>[],totals: Totals): (datum: PseudoDatum) => JSX.Element[] {
    var measuresArray = essence.getEffectiveMeasures().toArray();
    ...
    return (datum: PseudoDatum): JSX.Element[] => {
      ...
      return measuresArray.map((measure, i) => {
        ...
        if (datum['__nest'] === splitLength) {
          ...
          let pct =Math.round(10000*measureValue/totals[measure.name])/100;
          measureValueStr= measureValueStr+" ("+ pct +"%)";
					measureValueStr= React.createElement("div",{},measureValueStr);
					let backgroundWidth=pct;
  ...
  renderInternals() {
    ...
    if (flatData) {
      ...
      const measuresRenderer = this.makeMeasuresRenderer(essence, formatters, hScales,totals);
